...
class BackendB1GMail extends BackendDiff
{
    ...
    public function GetFolder($id)
    {
        ...
        // dates folder
        elseif (strlen($id) > 7 && substr($id, 0, 7) == '.dates:') {
        ...
        // tasks folder
        elseif (strlen($id) > 7 && substr($id, 0, 7) == '.tasks:') {
        ...
        // email folder
        elseif (strlen($id) > 7 && substr($id, 0, 7) == '.email:') {
    ...
    public function ChangeFolder($folderid, $oldid, $displayname, $type)
    {
        ...
        } elseif (strlen($folderid) > 7 && substr($folderid, 0, 7) == '.email:') {
            return $this->ChangeMailFolder($folderid, $oldid, $displayname, $type);
        } elseif (strlen($folderid) > 7 && substr($folderid, 0, 7) == '.dates:') {
    ...
    private function get_email_from_rfc_email($rfc_email_string)
    {
        ...
        if (filter_var($matches[1], FILTER_VALIDATE_EMAIL)) {
            return $matches[1];
        } elseif (filter_var($rfc_email_string, FILTER_VALIDATE_EMAIL)) {
    ...
    public function GetMessageList($folderid, $cutoffdate)
    {
        ...
        } elseif (strlen($folderid) > 7 && substr($folderid, 0, 7) == '.tasks:') {
    ...
    private function GetDate($folderid, $id, $contentparameters)
    {
        ...
        if (($row['repeat_flags'] & (8 | 16 | 32 | 64)) != 0) {
            ...
            // weekly
            elseif (($row['repeat_flags'] & 16) != 0) {
                $rec->type = 1;
                $rec->interval = max(1, $row['repeat_value']);
            }

            // monthly mday
            elseif (($row['repeat_flags'] & 32) != 0) {
                $rec->type = 3;
                $rec->interval = max(1, $row['repeat_value']);
            ...
            } elseif (($row['repeat_flags'] & 2) != 0) {	// count
    ...
    private function GetMail($folderid, $id, $contentparameters)
    {
        ...
        // parse addresses
        $result->to = $result->cc = $result->reply_to = [];
        $addresses = ['to' => [], 'cc' => [], 'reply-to' => []];
        if (!empty($parsedMail->headers['to'])) {
            $result->to = $this->ExplodeOutsideOfQuotation($parsedMail->headers['to'], [',', ';']);
        }
        if (!empty($parsedMail->headers['cc'])) {
            $result->cc = $this->ExplodeOutsideOfQuotation($parsedMail->headers['cc'], [',', ';']);
        }
        if (!empty($parsedMail->headers['reply-to'])) {
            $result->reply_to = $this->ExplodeOutsideOfQuotation($parsedMail->headers['reply-to'], [',', ';']);
        }
        ...
        if (Request::GetProtocolVersion() >= 12.0) {
            ...
            } elseif ($bodyPrefType == SYNC_BODYPREFERENCE_HTML) {
            ...
            } elseif ($bodyPrefType == SYNC_BODYPREFERENCE_MIME) {
                $asBodyData = $mailData;
        ...
        if ($bodyPrefType != SYNC_BODYPREFERENCE_MIME) {
            ...
            foreach ($attachments as $partID => $part) {
                ...
                } elseif (isset($part->d_parameters['name'])) {
                ...
                $attContentID = trim(str_replace(['<', '>'], '', $attContentID));
    ...
    private function GetContact($folderid, $id, $contentparameters)
    {
        ...
        if (!empty($row['vorname'])) {
            $result->firstname = $row['vorname'];
        }
        if (!empty($row['nachname'])) {
            $result->lastname = $row['nachname'];
        }

        if (!empty($row['tel'])) {
            $result->homephonenumber = $row['tel'];
        }
        if (!empty($row['fax'])) {
            $result->homefaxnumber = $row['fax'];
        }
        if (!empty($row['strassenr'])) {
            $result->homestreet = $row['strassenr'];
        }
        if (!empty($row['ort'])) {
            $result->homecity = $row['ort'];
        }
        if (!empty($row['plz'])) {
            $result->homepostalcode = $row['plz'];
        }
        if (!empty($row['land'])) {
            $result->homecountry = $row['land'];
        }

        if (!empty($row['work_strassenr'])) {
            $result->businessstreet = $row['work_strassenr'];
        }
        if (!empty($row['work_plz'])) {
            $result->businesspostalcode = $row['work_plz'];
        }
        if (!empty($row['work_ort'])) {
            $result->businesscity = $row['work_ort'];
        }
        if (!empty($row['work_land'])) {
            $result->businesscountry = $row['work_land'];
        }
        if (!empty($row['work_email'])) {
            $result->email2address = $row['work_email'];
        }
        if (!empty($row['work_tel'])) {
            $result->businessphonenumber = $row['work_tel'];
        }
        if (!empty($row['work_fax'])) {
            $result->businessfaxnumber = $row['work_fax'];
        }
        if (!empty($row['work_handy'])) {
            $result->business2phonenumber = $row['work_handy'];
        }

        if (!empty($row['email'])) {
            $result->email1address = $row['email'];
        }
        if (!empty($row['web'])) {
            $result->webpage = $row['web'];
        }
        if (!empty($row['handy'])) {
            $result->mobilephonenumber = $row['handy'];
        }
        if (!empty($row['firma'])) {
            $result->companyname = $row['firma'];
        }
        ...
        if (!empty($row['position'])) {
            $result->jobtitle = $row['position'];
        }
        if (!empty($row['geburtsdatum'])) {
            $result->birthday = $row['geburtsdatum'];
        }
    ...
    private function ChangeDate($folderid, $id, $date)
    {
        ...
        if (isset($date->recurrence) && is_object($date->recurrence)) {
            ...
            // ...or n times
            elseif (isset($rec->occurences) && $rec->occurences > 0) {
                $row['repeat_flags'] |= 2;
    ...
    private function ChangeTask($folderid, $id, $task)
    {
        ...
        // update existing item
            ...
            if ($task->complete) {
                $row['akt_status'] = 64;
            } elseif ($row['akt_status'] == 64) {
                $row['akt_status'] = 16;
            }

            if ($task->startdate > 0) {
                $row['beginn'] = $task->startdate;
            }
            if ($task->duedate > 0) {
                $row['faellig'] = $task->duedate;
            }
    ...
    private function GuessPictureType($data)
    {
        if (substr($data, 0, 4) == "\x47\x49\x46\x38") {
            return 'image/gif';
        } elseif (substr($data, 0, 4) == "\xFF\xD8\xFF\xE0") {
            return 'image/jpg';
        } elseif (substr($data, 0, 4) == "\x89\x50\x4E\x47") {
            return 'image/png';
        } else {
    ...
    private function GetTextFromParsedMail($parsedMail, $type)
    {
        ...
        while (count($objs) > 0) {
            ...
            if (strtolower($obj->ctype_primary) == 'text' && strtolower($obj->ctype_secondary) == strtolower($type)) {
                $result .= $obj->body;
            } elseif (strtolower($obj->ctype_primary) == 'multipart' && !empty($obj->parts)) {
    ...
    private function GetAttachmentsFromParsedMail($parsedMail)
    {
        ...
        while (is_array($objs) && count($objs) > 0) {
            ...
            if (strtolower($part->ctype_primary) == 'multipart' && in_array(strtolower($part->ctype_secondary), ['alternative', 'mixed', 'related'])) {
            ...
            if (!empty($part->disposition) && !in_array($part->disposition, ['attachment', 'inline'])) {
                continue;
            }
    ...
    {
        $senders = [strtolower($this->userRow['email'])];
     ...
     */
    public function CreateBlobStorageProvider($id)
    {
        $result = false;
     ...
     * @return string
     */
    public function GetMessageData($id, $blobStorage)
    {
